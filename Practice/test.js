...
describe("generate practice test", function() {
    ...
    if (result.length > 5) {
      it(`generated data must contain an input empty array`, function() {
        expect(result.find(e => e.input === [])).not.toBeUndefined();
      });
      it(`generated data must contain an input with target not found`, function() {
        expect(result.find(e => e.input.indexOf(e.target))).toBeUndefined();
      });
      ...
      it(`generated data must contain target at last index`, function() {
        expect(result.find(e => e.input[e.length - 1] === e.target)).not.toBeUndefined();
      });
    ...
    it(`generated output data must equal to data from the file`, function() {
      var outputFileData = JSON.parse(fs.readFileSync("./Practice3/test-data.json", "utf-8"));
      expect(result === outputFileData).toBe(true);
    })
});
